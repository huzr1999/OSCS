...
from utils.plot import plot_and_save_training_loss_curve
import yaml
...
class Trainer(object):
    ...
    def __init__(
        ...
        # dataset_name: Optional[str] = "sst-2",
    ...
    def register(self, model: Victim, dataloader, metrics):
        ...
        self.logger.info("  Instantaneous batch size per GPU = %d", self.batch_size)
        self.logger.info("  Gradient Accumulation steps = %d", self.gradient_accumulation_steps)
        self.logger.info("  Total optimization steps = %d", self.epochs * train_length)
    ...
    def train(self, model: Victim, dataset, metrics: Optional[List[str]] = ["accuracy"]):
        ...
        for epoch in range(self.epochs):
            ...
            self.logger.info('Epoch: {}, avg loss: {}'.format(epoch+1, epoch_loss))

        self.logger.info("Training finished.")
        # dev_results, dev_score = self.evaluate(self.model, eval_dataloader, self.metrics)
        # self.logger.info("***** Evaluate model on dev set *****")
        ...
        self.logger.info("Saving model to {}".format(self.save_path))
        os.makedirs(self.save_path, exist_ok=True)
        self.model.plm.save_pretrained(self.save_path)
        yaml.dump(self.train_configs, open(os.path.join(self.save_path, 'config_used.yaml'), 'w'))

        if len(self.losses) > 0:
            plot_and_save_training_loss_curve([self.losses], ["total_loss"])
        if len(self.act_losses) > 0 and len(self.rep_losses) > 0:
            plot_and_save_training_loss_curve([self.cls_losses, self.act_losses, self.rep_losses], ["classification_loss", "activation_regularization_loss", "representation_regularization_loss"])
    ...
    def compute_hidden(self, model: Victim, dataloader: torch.utils.data.DataLoader):
        ...
        """
        self.logger.info('***** Computing hidden hidden_state *****')
        model.eval()
    ...
    def visualization(self, hidden_states: List, labels: List, poison_labels: List, fig_basepath: Optional[str]="./visualization", fig_title: Optional[str]="vis"):
        ...
        for epoch in tqdm(range(self.epochs+1)):
            ...
            self.logger.info(f'Saving png to {fig_path}')
            plt.close()
    ...
    #     return os.path.join(self.save_path, f'{settings.timestamp_str}')
